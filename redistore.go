...
// TODO : Change above text and add method to store a session by user ID and methods for getting
...
func (s *RediStore) Save(r *http.Request, w http.ResponseWriter, session *sessions.Session) error {
	...
	if session.Options.MaxAge < 0 {
		...
		encoded, err := s.GetEncoded(session)
		if err != nil {
			return err
		}
...
// GetEncoded gets the encoded cookie value of the given session
func (s *RediStore) GetEncoded(session *sessions.Session) (encoded string,err error) {
